...
# Minimal G$ ERC-20 ABI used for direct-transfer disbursement.
_GD_ERC20_ABI = [
    {
        "constant": True,
...
class TrustpilotTaskBlockchain:
    """Trustpilot Task Disbursement via direct G$ ERC-20 transfer."""
    ...
    def __init__(self):
        ...
        if self.w3.is_connected():
            logger.info("✅ Connected to Celo network for Trustpilot Task")
        else:
            logger.error("❌ Failed to connect to Celo network")

        logger.info("⭐ Trustpilot Task Blockchain Service initialized (DAILYTASK_KEY direct-transfer mode)")
    ...
    async def disburse_trustpilot_reward(self, wallet_address: str, amount: float, task_id: str = None) -> dict:
        """
        Disburse Trustpilot Task reward as a direct G$ ERC-20 transfer signed by
        ...
        try:
            ...
            logger.info(f"⭐ Trustpilot reward disbursement: {amount} G$ to {masked_wallet} | task_id={task_id}")
            ...
            # Pre-flight: verify the sender wallet has enough CELO for gas
            try:
            ...
            # Pre-flight: verify sufficient G$ balance
            try:
            ...
            try:
                ...
                try:
                    ...
                    logger.info(f"⛽ Trustpilot reward gas estimate: {estimated_gas} (using limit: {gas_limit})")
                except Exception:
                    gas_limit = 250000
                    logger.warning(f"⚠️ Gas estimation failed, falling back to {gas_limit}")
            ...
            try:
                ...
                logger.info(f"📤 Trustpilot reward transfer sent: {tx_hash_hex}")
            ...
            if receipt.status == 1:
                logger.info(
                    f"✅ Trustpilot reward disbursed: {reward_amount_wei / 10**18} G$ "
            ...
            logger.error(f"❌ Trustpilot reward transfer reverted on-chain | tx={tx_hash_hex}")
            return {
                "success": False,
            ...
            logger.error(f"❌ Trustpilot reward disbursement error: {e}")
            import traceback
            logger.error(f"🔍 Traceback: {traceback.format_exc()}")
            return {"success": False, "error": str(e)}

    def disburse_trustpilot_reward_sync(self, wallet_address: str, amount: float, task_id: str = None) -> dict:
        """Synchronous wrapper for disburse_trustpilot_reward"""
        import asyncio
        ...
        try:
            try:
                ...
                return asyncio.run(self.disburse_trustpilot_reward(wallet_address, amount, task_id))
        except Exception as e:
    ...
    def _run_in_new_loop(self, wallet_address: str, amount: float, task_id: str = None) -> dict:
        ...
        try:
            return loop.run_until_complete(self.disburse_trustpilot_reward(wallet_address, amount, task_id))
        finally:
            loop.close()


# Global instance
trustpilot_blockchain_service = TrustpilotTaskBlockchain()
