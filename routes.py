...
def _user_language_row(wallet):
    """Return the user_data row carrying language state (case-insensitive).

    Mirrors the repo convention: web sessions hold a checksummed wallet while
    other writers may store lowercase, so never use `.eq('wallet_address', ...)`.
    """
    try:
        from flask import g

        cached = getattr(g, "_gm_language_row", None)
        ...
        if supabase:
            result = safe_supabase_operation(
                lambda: supabase.table("user_data")
                    .select("wallet_address, language, language_picked_at")
                    .ilike("wallet_address", wallet)
                    .limit(1)
                    .execute(),
                operation_name="fetch user language"
            )
            if result and result.data:
                row = result.data[0]
    except Exception as exc:
        logger.warning(f"[language] lookup failed: {exc}")

    try:
        from flask import g

        g._gm_language_row = row
    except Exception:
        pass
    return row


def _user_language_context(wallet):
    """(label, picked) for the wallet page.

    The DB is consulted only when the session has not recorded a choice yet,
    so an already-picked user costs no extra query per page view.
    """
    import i18n

    picked_session = bool(session.get("language_picked"))
    language = i18n.normalize_language(session.get("language"))
    picked_from_db = False

    if not (language and picked_session):
        row = _user_language_row(wallet)
        if row:
            language = language or i18n.normalize_language(row.get("language"))
            picked_from_db = bool(row.get("language_picked_at"))

    language = language or i18n.DEFAULT_LANGUAGE
    return i18n.LANGUAGE_LABELS.get(language, "English"), (picked_session or picked_from_db)


@routes.route("/api/user/language", methods=["GET"])
@auth_required
def get_user_language():
    """Current language, the supported list, and whether the pick was made."""
    import i18n

    wallet = session.get("wallet")
    row = _user_language_row(wallet)
    language = i18n.resolve_language(
        session=session,
        user_row=row,
    ...
    return jsonify({
        "success": True,
        "language": language,
        "picked": bool(row and row.get("language_picked_at")) or bool(session.get("language_picked")),
        "supported": [{"code": c, "label": l} for c, l in i18n.SUPPORTED_LANGUAGES],
    })


@routes.route("/api/user/language", methods=["POST"])
@auth_required
def set_user_language():
    """Persist the language choice.

    The first save stamps `language_picked_at`, which is what stops the
    one-time prompt from reappearing on sign-in. Later changes are allowed —
    the pick is one-time as a *prompt*, not a permanent lock.
    """
    import i18n
    ...
    if not language:
        return jsonify({
            "success": False,
            "error": "Unsupported language. Choose one of: " + ", ".join(i18n.SUPPORTED_CODES),
        }), 400

    session["language"] = language
    session["language_picked"] = True

    row = _user_language_row(wallet)
    already_stamped = bool(row and row.get("language_picked_at"))
    persisted = False
    try:
        supabase = get_supabase_client()
        if supabase:
            ...
            if not already_stamped:
                update["language_picked_at"] = datetime.now(timezone.utc).isoformat()
            result = safe_supabase_operation(
                lambda: supabase.table("user_data")
                    .update(update)
                    .ilike("wallet_address", wallet)
                    .execute(),
                operation_name="update user language"
            )
            persisted = bool(result)
    except Exception as exc:
        # A DB hiccup must not lose the choice for this session — the session
        # copy is applied on the next request either way.
        logger.warning(f"[language] persist failed: {exc}")
...
@routes.route("/api/i18n/catalog", methods=["GET"])
def i18n_catalog():
    """Dictionary for the active (or requested) language.

    Public on purpose: it only contains the app's own UI strings, and the
    client runtime needs it before any auth-gated data. Returns an EMPTY map
    for English so the runtime short-circuits.
    """
    import i18n

    language = (
        i18n.normalize_language(request.args.get("lang"))
        or i18n.normalize_language(session.get("language"))
        or i18n.DEFAULT_LANGUAGE
    )
    translations = {} if language == i18n.DEFAULT_LANGUAGE else i18n._load_dictionary(language)
    resp = jsonify({
        "success": True,
        "language": language,
        "translations": translations,
    })
    # Safe to cache briefly: bump ASSET_VERSION to invalidate after edits.
...
def update_quiz_settings():
    ...
    try:
        ...
        if cooldown_hours is not None and (cooldown_hours < 1 or cooldown_hours > 720):
            return jsonify({
                "success": False,
                "error": "Quiz cooldown must be between 1 and 720 hours"
            }), 400
...
def wallet_page():
    ...
    return render_template(
        ...
        language_picked=_gm_lang_label_and_picked(wallet)[1],
        GM_LANGUAGE_LABEL=_gm_lang_label_and_picked(wallet)[0],
    )


def _gm_lang_label_and_picked(wallet):
    from flask import g

    cached = getattr(g, "_gm_lang_ctx", None)
    if cached is None:
        cached = _user_language_context(wallet)
        g._gm_lang_ctx = cached
    return cached
