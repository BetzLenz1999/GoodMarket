...
const WALLET = window.GM_WALLET_BOOT.wallet;
    ...
    // ── Language picker (one-time on first visit + changeable in Settings) ──
    // The chosen code lives on `sessionStorage` for instant feedback and on the
    // server (session + user_data) as the source of truth. We reload after a
    // save so server-rendered text re-resolves in the new language.
    var GM_SUPPORTED_LANGUAGES = [['en', 'English'], ['fil', 'Filipino'], ['es', 'Español']];
    var _gmLangPending = null;

    function _gmLangCurrent() {
        var boot = window.GM_WALLET_BOOT || {};
        return (boot.language || 'en').toLowerCase();
    }

    function _gmLangRender() {
        var host = document.getElementById('gmLangOptions');
        if (!host) return;
        var current = _gmLangPending || _gmLangCurrent();
        host.innerHTML = GM_SUPPORTED_LANGUAGES.map(function (pair) {
            var code = pair[0], label = pair[1];
            var sel = code === current;
            return '<button type="button" class="gm-lang-option' + (sel ? ' is-selected' : '') + '"' +
                ' role="radio" aria-checked="' + sel + '" data-lang="' + code + '"' +
                ' onclick="selectLanguageOption(\'' + code + '\')">' +
                '<span>' + label + '</span>' +
                '<span class="gm-lang-code">' + code +
                (sel ? ' <span class="gm-lang-check">✓</span>' : '') + '</span>' +
                '</button>';
        }).join('');
        var btn = document.getElementById('gmLangSaveBtn');
        if (btn) {
            // Nothing chosen yet -> make clear this saves the highlighted one.
            btn.disabled = false;
            btn.textContent = _gmLangPending ? 'Continue' : 'Keep ' + _gmLangLabel(current);
        }
    }

    function _gmLangLabel(code) {
        for (var i = 0; i < GM_SUPPORTED_LANGUAGES.length; i++) {
            if (GM_SUPPORTED_LANGUAGES[i][0] === code) return GM_SUPPORTED_LANGUAGES[i][1];
        }
        return 'English';
    }
    ...
    // Deliberately bypasses openModal(): that closes every other overlay, and
    // changing the language from Settings must leave Settings open behind it.
    function openLanguageModal() {
        _gmLangPending = null;
        _gmLangRender();
        document.querySelectorAll('.modal-overlay.open').forEach(function (el) {
            if (el.id !== 'languageModal' && el.id !== 'lwUnlockModal') el.classList.remove('open');
        });
    ...
    function saveLanguageChoice() {
        var target = _gmLangPending || _gmLangCurrent();
        if (target === _gmLangCurrent()) {
            // Already on this language: just record the one-time pick so the
            // prompt stops appearing, without a needless reload.
            try { sessionStorage.setItem('gmLanguage', target); } catch (e) { }
            if (window.GMi18n && window.GMi18n.setLanguage) {
                window.GMi18n.setLanguage(target, { reload: false }).then(closeLanguageModal).catch(closeLanguageModal);
            } else {
                ...
                }).then(closeLanguageModal).catch(closeLanguageModal);
            }
            return;
        }
        var btn = document.getElementById('gmLangSaveBtn');
        ...
        if (window.GMi18n && window.GMi18n.setLanguage) {
            window.GMi18n.setLanguage(target).catch(function () { window.location.reload(); });
        } else {
            fetch('/api/user/language', {
                method: 'POST',
                credentials: 'same-origin',
    ...
    window.saveLanguageChoice = saveLanguageChoice;

    // First-visit prompt: only while the user has never picked a language.
    (function _gmLangMaybePrompt() {
        var boot = window.GM_WALLET_BOOT || {};
        if (boot.languagePicked) return;
        var run = function () { openLanguageModal(); };
    ...
    (function() {
        ...
        window._lwUnlockSubmit = async function () {
            ...
            try {
                ...
                // Shared recovery: a missing local keystore (private mode,
                // cleared site data, new browser profile) falls back to the
                // server copy instead of dead-ending on "No saved wallet
                // found on this device."
                var result = await GMLocalWallet.handleUnlock(pin, saved);
                if (!result.ok) {
                    var expected = GMLocalWallet.expectedWalletAddress();
                    if (saved && saved.address && expected &&
                        saved.address.toLowerCase() !== expected.toLowerCase()) {
                        try { GMLocalWallet.clearLocalKeystore(); } catch (_) {}
                    }
                    throw new Error(GMLocalWallet.describeUnlockError(result.err, saved));
                }
                ...
                if (errEl) {
                    errEl.textContent = (err && err.message) || 'Unlock failed.';
